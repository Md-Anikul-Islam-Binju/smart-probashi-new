            ...
            <div class="card card-flush">
                <div class="card-body pt-0">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="">
                        <form action="{{ route('admin.job.management') }}" method="GET" >
                            <div class="row">
                        ...
                        @foreach($jobs as $key=>$jobsData)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>
                                    Name: {{$jobsData->recruitingAgency? $jobsData->recruitingAgency->name:'N/A'}}
                                    <br>
                                    Email: {{$jobsData->recruitingAgency? $jobsData->recruitingAgency->email:'N/A'}}
                                    <br>
                                    Phone: {{$jobsData->recruitingAgency? $jobsData->recruitingAgency->phone:'N/A'}}
                                </td>
                                <td>{{$jobsData->jobCategory? $jobsData->jobCategory->job_category_name:'N/A'}}</td>
                                <td>{{$jobsData->no_of_post}}</td>

                                <td>
                                    @if($jobsData->salary_amount!=null)
                                        {{$jobsData->salary_amount}}/Month
                                    @else
                                        N/A
                                    @endif
                                </td>
                                ...
                                <td>
                                    <a class="btn btn-primary btn-sm"  href="{{route('admin.job.details',$jobsData->id)}}">View</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
